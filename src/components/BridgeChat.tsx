...
import { Send, Mic, Radio, Volume2, Activity } from 'lucide-react';
...
// Audio assets for immersion (using the same static assets as DispatcherChat)
...
const BridgeChat: React.FC<BridgeChatProps> = ({ mission }) => {
    const { sendChatRelay } = useLocalBridge();
    ...
    // Mock function to play audio (since we don't have the TTS audio URL in the bridge)
    const playMockAudio = () => {
        ...
        setTimeout(() => {
            staticAudio.pause();
            // Simulate dispatcher response time
            setTimeout(() => {
                staticAudio.currentTime = 0;
                staticAudio.play();
    ...
    const addLog = (sender: LogEntry['sender'], message: string, callsign?: string) => {
    ...
    const handleSend = async (manualText?: string) => {
        ...
        addLog('Crew', text, callsign);
        setInput('');
        setIsProcessing(true);

        // Use the local bridge proxy to send the message to the Supabase Edge Function
        const response = await sendChatRelay(mission.missionId, text);
        
        setIsProcessing(false);
        if (response?.response_text) {
            addLog('Dispatcher', response.response_text);
            playMockAudio(); // Play mock audio cue
        } else {
            toast.error("Dispatch Agent failed to respond.");
        }
    };
                ...
                <div className="flex justify-between items-center">
                    <div className="flex flex-col">
                        <CardTitle className="text-[10px] tracking-[0.2em] uppercase font-black italic flex items-center">
                            <Radio className="w-3 h-3 mr-2 text-primary" /> COMSAT_UPLINK_v5.3
                        </CardTitle>
